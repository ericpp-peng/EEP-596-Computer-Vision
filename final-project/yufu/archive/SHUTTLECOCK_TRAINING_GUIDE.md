...
### ⚠️ 標註工具說明
我們使用內建的 `simple_annotator.py`，**不需要安裝 labelImg**。

`simple_annotator.py` 已包含在專案中，具備：
- ✅ 滑鼠拖曳標註框
- ✅ YOLO 格式自動儲存
- ✅ 載入已有標註
- ✅ 鍵盤快捷鍵操作
...
#### 標註完成後
- 標註會自動儲存到 `badminton_ball_dataset/labels/train/`
- 格式為 YOLO 格式（`.txt` 檔）
- 每個圖片對應一個 `.txt` 檔（例如：`frame_0001.txt`）
