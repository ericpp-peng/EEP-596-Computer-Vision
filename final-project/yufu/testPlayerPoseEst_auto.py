"""
羽球影片分析系統 - 自動預測模式 (Auto Mode)
============================================

本檔案是 testPlayerPoseEst.py 的自動版本：
- 🤖 完全自動預測，不需要用戶輸入答案
- 📹 從頭到尾連續處理影片
- 🎯 自動偵測球、追蹤軌跡、判斷擊球類型
- 📊 保存預測結果供後續分析
...
如需訓練模型或收集標註數據，請使用 testPlayerPoseEst.py
"""
...
# 輸出影片路徑（可被命令列參數覆蓋）
OUTPUT_PATH = "./output_with_pose_auto.mp4"  # ⭐ 改名以區分自動模式輸出
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='''
範例:
  python testPlayerPoseEst_auto.py                              # 使用預設影片
  python testPlayerPoseEst_auto.py -i input.mp4                 # 指定輸入影片
  python testPlayerPoseEst_auto.py -i input.mp4 -o output.mp4   # 指定輸入和輸出
        ''')
    ...
    while True:
        ...
        if selected_person and kpts_all_cpu is not None:
            ...
            if arm_raised_frame is not None and not analysis_complete and (frame_idx - arm_raised_frame) >= 55:
                ...
                if ball_at_raise and ball_now:
                    ...
                    # ⭐ 自動模式：直接接受 AI 判斷，不需要用戶輸入
                    user_label = shot_type
                    print(f"✅ 自動接受 AI 判斷: {shot_type}\n")
        ...
        # 即時顯示處理結果
        cv2.imshow('Badminton Analysis (Auto Mode)', frame)
        
        # 按 'q' 可以提前結束（自動模式：移除暫停功能）
